...
describe("live booking verification", () => {
  ...
  it("does not read Calendar even if it would report conflicts or fail", async () => {
    calendarFetch.mockRejectedValue(new Error("Provider unavailable"));
    await expect(verify()).resolves.toBeDefined();
    expect(calendarFetch).not.toHaveBeenCalled();
  });
  it("keeps unavailable Wise evidence blocked and retryable", async () => {
    vi.mocked(fetchWiseSessionsForBangkokDates).mockRejectedValue(
      new Error("429"),
    );
