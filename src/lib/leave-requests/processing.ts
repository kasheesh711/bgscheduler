...
import { normalizeLeave, type normalizationInput } from "./normalization";

/** Each result commits before another batch starts. A killed route resumes here. */
export async function processLeaveNormalizations(db: Database, options: { budgetMs?: number; normalize?: typeof normalizeLeave; now?: Date } = {}) {
  ...
  const work = await db.select({ revision: s.leaveNormalizations, endDate: s.leaveRequests.endDate, startDate: s.leaveRequests.startDate })
    ...
    .where(and(inArray(s.leaveNormalizations.status, ["pending", "failed"]), or(sql`${s.leaveNormalizations.nextAttemptAt} is null`, lte(s.leaveNormalizations.nextAttemptAt, now))));
  ...
  return { processed, failed, remaining: work.length - processed - failed };
}
