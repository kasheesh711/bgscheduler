...
export const commandSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("create-paper"), title: z.string().trim().min(1).max(300), ownerKey: z.string().optional() }).strict(),
  ...
  z.object({ action: z.literal("upload-intent"), ownerKey: z.string().optional(), name: z.string().min(1).max(250), mime: z.enum(SOURCE_TYPES), size: z.number().int().positive().max(MAX_FILE_BYTES), purpose: z.enum(["paper", "key", "work"]) }).strict(),
