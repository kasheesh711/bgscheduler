...
describe("buildPackageExportRows", () => {
  it("combines package-band summaries and additional revenue mix in one export", () => {
    const bands = summarizePackageBands([
      makePackageAgg({ packageBand: "20h", packageLabel: "20 Hours", rev: 40_000, count: 2 }),
    ]);
    ...
    expect(rows).toEqual([
      expect.objectContaining({ section: "Package bands", label: "20h", revenue: 40_000, txnCount: 2 }),
      expect.objectContaining({ section: "Additional revenue", label: "Books", revenue: 1_500, txnCount: 3 }),
    ]);
  });
});
