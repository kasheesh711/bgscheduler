...
export function LeaveRequestsWorkspace() {
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain" data-testid="leave-work-scroll">
    <div className="mx-auto w-full max-w-6xl space-y-5 pb-12 pt-2">
      ...
      {board && (board.freshness.stale || board.freshness.errors.length || board.freshness.pendingNormalization || board.freshness.failedNormalization) ? <div role="status" className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200"><p className="flex items-start gap-2 font-medium"><AlertCircle className="mt-0.5 size-4 shrink-0" />{board.freshness.errors.find((message) => message.includes("API credits are exhausted")) || (board.freshness.stale ? "Source data is stale. Saved assignments and checkoffs are retained." : board.freshness.pendingNormalization ? `${board.freshness.pendingNormalization} submissions are being interpreted automatically.` : "Some submissions could not be processed. Retries are automatic.")}</p><p className="mt-1 pl-6 text-xs">Last source read: {board.freshness.sourceReadAt ? new Date(board.freshness.sourceReadAt).toLocaleString("en-GB", { timeZone: "Asia/Bangkok", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "not yet available"}{board.freshness.pendingWritebacks ? ` · ${board.freshness.pendingWritebacks} sheet summaries waiting to sync` : ""}</p>{board.freshness.errors.length ? <details className="mt-2 pl-6"><summary className="cursor-pointer text-xs font-medium">Show {board.freshness.errors.length} processing issues</summary><ul className="mt-2 list-disc space-y-1 pl-4 text-xs">{board.freshness.errors.map((message, i) => <li key={i}>{message}</li>)}</ul></details> : null}</div> : null}
      ...
      {board && !assignments.length && view !== "history" && !board.processingRequests?.length ? <div className="rounded-xl border border-dashed border-border px-5 py-12 text-center"><div className="mx-auto mb-3 flex size-10 items-center justify-center rounded-full bg-primary/10 text-primary"><Check className="size-5" /></div><p className="font-medium">{selectedOwner === "unassigned" ? "No unassigned work for this view" : "No assignments in this view"}</p><p className="mt-1 text-sm text-muted-foreground">{view === "upcoming" ? "Future class cancellations will appear here as submissions are processed." : "Check Everyone for shared work, or Upcoming for later processing dates."}</p>{selectedOwner !== "everyone" ? <Button className="mt-4" variant="outline" onClick={() => setOwner("everyone")}>See everyone’s work</Button> : null}</div> : null}
      {view !== "history" && board?.processingRequests?.length ? <details open={!board.assignments.length} className="rounded-xl border border-amber-200 bg-card p-4 dark:border-amber-900"><summary className="cursor-pointer text-sm font-semibold">{board.processingRequests.length} upcoming submissions still processing</summary><p className="mt-2 text-xs text-muted-foreground">These submissions are saved. Assignments appear automatically when interpretation and teacher matching finish.</p><div className="mt-3 divide-y divide-border">{board.processingRequests.map((r) => <button type="button" key={r.id} className="flex w-full items-center justify-between gap-3 py-3 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => setTarget({ id: r.id, title: r.teacher, request: true })}><span className="min-w-0"><span className="block text-sm font-medium">{r.teacher}</span><span className="mt-1 block text-xs text-muted-foreground">{r.startDate ? formatWorkDate(r.startDate) : "Date unresolved"}{r.endDate && r.endDate !== r.startDate ? ` – ${formatWorkDate(r.endDate)}` : ""} · {r.status}</span></span><ChevronRight className="size-4 shrink-0" /></button>)}</div></details> : null}
