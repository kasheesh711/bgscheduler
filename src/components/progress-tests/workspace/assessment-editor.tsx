...
export function AssessmentEditor({ data, overview, onBack, onSaved, onError }: { data: Serialized<AssessmentDetail>; overview: Serialized<Overview>; onBack: () => void; onSaved: () => Promise<void>; onError: (message: string) => void }) {
  ...
  return <>
    <div className={css.detailHeader}><div><Button variant="quiet" onClick={onBack}><ArrowLeft size={15}/>Back to workspace</Button><h2>{data.series.studentName}</h2><p>{data.series.courseName} · {data.series.tutorName} · Cycle {data.cycle} · Due in class {data.dueClass}</p></div><div className={css.actions}><span className={`${css.badge} ${data.overdue ? css.warning : ""}`}>{STAGE_LABELS[data.stage]}{data.overdue ? " · Overdue" : ""}</span><Button variant="quiet" aria-label="Refresh saved results" onClick={() => { void onSaved().catch(e => onError(e.message)); }}><RefreshCw size={16}/></Button></div></div>
