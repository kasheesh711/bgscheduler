...
import { ArrowLeft, CheckCheck, ClipboardCheck, FileText, Sparkles } from "lucide-react";
import type { AssessmentDetail, Overview } from "@/lib/progress-tests/workspace/data";
import { emptyReport, validateMarks, type Mark, type PageRef, type Report } from "@/lib/progress-tests/workspace/model";
import type { Command } from "@/lib/progress-tests/workspace/commands";
import { Button, Field, PageOrder, UploadField, useWorkspaceTransport, formatDate, type Serialized, type Uploaded } from "./shared";
...
export function AssessmentEditor({ data, overview, onBack, onSaved, onError }: { data: Serialized<AssessmentDetail>; overview: Serialized<Overview>; onBack: () => void; onSaved: () => Promise<void>; onError: (message: string) => void }) {
  const {command,fileUrl,sampleUpload}=useWorkspaceTransport();
  const current = data.reviews.find(r => r.id === data.currentReviewId);
  const currentPaper = data.versions.find(v => v.version.id === data.preparation.paperVersionId)?.version.paper;
  const [paperId, setPaperId] = useState(data.preparation.paperVersionId ?? "");
  const [topics, setTopics] = useState(data.preparation.topics);
  const [informed, setInformed] = useState(data.preparation.studentInformed);
  const [sessionId, setSessionId] = useState("");
  const [files, setFiles] = useState<Uploaded[]>([]);
  const [order, setOrder] = useState<PageRef[]>([]);
  const [marks, setMarks] = useState<Mark[]>(current?.data.marks ?? currentPaper?.questions.map(q => ({ questionId:q.id, marks:0, explanation:"", answerReference:"", needsReview:true })) ?? []);
  const [report, setReport] = useState<Report>(current?.data.report ?? { ...emptyReport(), contextLimitations: data.feedback.length ? "" : "Verified class feedback from this tutor was unavailable for this cycle." });
  const [confirmed, setConfirmed] = useState(false);
  const [busy, setBusy] = useState(false);
  const [preview, setPreview] = useState<string | null>(null);
  const [message, setMessage] = useState("");
  const sameReview = !!current && JSON.stringify(current.data.marks) === JSON.stringify(marks) && JSON.stringify(current.data.report) === JSON.stringify(report);
  const artifacts = data.artifacts.filter(a => a.reviewId === data.currentReviewId);
  const totals = (() => { try { return currentPaper ? validateMarks(currentPaper,marks) : null; } catch { return null; } })();
  ...
  return <>
    <div className={css.detailHeader}><div><Button variant="quiet" onClick={onBack}><ArrowLeft size={15}/>Back to workspace</Button><h2>{data.series.studentName}</h2><p>{data.series.courseName} · {data.series.tutorName} · Cycle {data.cycle} · Due in class {data.dueClass}</p></div><span className={`${css.badge} ${data.overdue ? css.warning : ""}`}>{STAGE_LABELS[data.stage]}{data.overdue ? " · Overdue" : ""}</span></div>
    <div className={css.stepList}>{Object.entries(STAGE_LABELS).map(([stage,label]) => <span data-active={stage === data.stage} key={stage}>{label}</span>)}</div>
    ...
    <div className={css.detailGrid}><div>
      <section className={css.panel}><h3>1. Prepare the test <small>Discuss in class {data.discussionClass}</small></h3>
        <Field label="Reviewed paper and rubric"><select value={paperId} disabled={!!data.currentSubmissionId} onChange={e => setPaperId(e.target.value)}><option value="">Choose a paper from your library</option>{data.versions.map(v => <option value={v.version.id} key={v.version.id}>{v.version.paper.title} · version {v.version.revision}</option>)}</select></Field>
      ...
      <section className={css.panel}><h3>2. Submit student work <small>Within an ordinary class</small></h3><p className={css.hint}>Record the class in which you administered the test. A late test does not move the next eight-class deadline.</p>
        ...
        <UploadField purpose="work" ownerKey={data.series.ownerKey} disabled={!overview.capabilities.uploads || !data.preparation.paperVersionId} onUploaded={uploaded => { setFiles([...files,...uploaded]); setOrder([...order,...uploaded.flatMap(f => Array.from({ length:f.pageCount },(_,i) => ({ fileId:f.id,page:i+1 })))]); }}/>
      ...
      {data.currentSubmissionId && currentPaper && <section className={css.panel}><h3>3. Review question marks</h3><p className={css.hint}>AI provides a draft. Check it against the original responses and approved rubric, then resolve every flagged answer. You can enter all marks manually.</p>
        <div className={css.actions}><Button disabled={busy || !overview.capabilities.ai} onClick={() => run({ action:"grade",...target })}><Sparkles size={15}/>{current ? "Create a new AI grading draft" : "Grade with AI"}</Button>{totals && <div className={css.resultScore}>{totals.earned} / {totals.possible} <small>({totals.percent}%)</small></div>}</div>
        ...
        <Button disabled={busy} onClick={() => run({ action:"save-review",...target,marks,report })}>Save reviewed marks</Button>
      </section>}
      {data.currentSubmissionId && currentPaper && <section className={css.panel}><h3>4. Write the progress report</h3><p className={css.hint}>Class feedback adds learning context and recommendations. It never changes the numerical test marks.</p>
        <Button disabled={busy || !overview.capabilities.ai || !sameReview || marks.some(m => m.needsReview)} onClick={() => run({ action:"report",...target })}><Sparkles size={15}/>Draft report from saved results</Button>
        ...
        <Button variant="primary" disabled={busy} onClick={() => run({ action:"save-review",...target,marks,report })}>Save report and marks</Button>
      </section>}
    </div><aside>
      <section className={css.panel}><h3><FileText size={18}/>Document preview</h3><div className={css.actions}>{artifacts.map(a => <Button key={a.id} onClick={() => setPreview(a.fileId)}>{a.kind === "graded" ? "Graded test" : "Progress report"}</Button>)}</div>
        {preview ? <><iframe className={css.pdfPreview} title="Assessment document preview" src={fileUrl(preview)}/><a className={css.fileLink} href={fileUrl(preview)} target="_blank" rel="noreferrer">Open document in a new tab ↗</a></> : <div className={css.empty}><FileText/><p>Open original work or generate both reviewed PDFs.</p></div>}
        {current && <><Button disabled={busy || !overview.capabilities.uploads || !sameReview} onClick={() => run({ action:"preview-review",...target })}><FileText size={15}/>Build both PDF previews</Button><label className={css.check}><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)}/>I reviewed every page of the graded test and progress report and approve these results.</label>
      ...
      <section className={css.panel}><h3><ClipboardCheck size={18}/>Class feedback</h3><p className={css.hint}>Only verified feedback from this tutor, student and course cycle is included.</p><div className={css.versionList}>{data.feedback.length ? data.feedback.map(f => <div key={f.id}><strong>{formatDate(f.date)}</strong><p style={{ whiteSpace:"pre-wrap" }}>{f.text}</p></div>) : <p>No verified feedback is available for this cycle. Record this limitation in the report.</p>}</div></section>
      <section className={css.panel}><h3>Review history</h3><p className={css.hint}>Wise destination: Content → Progress Tests. Earlier approved versions are retained.</p>
      {data.publications.map(p=><div className={css.notice} key={p.id}><div><strong>Version {p.version} · {p.status.replaceAll("_"," ")}</strong><p>{p.error}</p>{data.publicationFiles.filter(f=>f.publicationId===p.id).map(f=><p key={f.id}>{f.kind==="graded"?"Graded test":"Progress report"}: {f.status==="verified"?"Verified in Wise":f.status}</p>)}{p.status!=="published"&&<Button disabled={busy} onClick={()=>run({action:"publish",...target,publicationId:p.id})}>Check Wise and retry</Button>}{p.status==="published"&&!sampleUpload&&<a href={`https://learn.begiftededucation.com/teacher/classes/${data.series.wiseClassId}/overview?type=one_to_one&tab=content`} target="_blank" rel="noreferrer">Open Wise Content ↗</a>}</div></div>)}<div className={css.versionList}>{data.reviews.map((r,i) => <div key={r.id}><strong>{r.approved ? "Approved" : "Draft"} · {formatDate(r.createdAt)}</strong><p>{i===0 ? "Latest review" : "Earlier review"} · {r.data.feedback.length} feedback references</p><div className={css.actions}>{data.artifacts.filter(a => a.reviewId===r.id).map(a => <Button key={a.id} variant="quiet" onClick={() => setPreview(a.fileId)}>{a.kind === "graded" ? "Graded test" : "Report"}</Button>)}</div></div>)}</div></section>
    </aside></div>
  </>;
}
