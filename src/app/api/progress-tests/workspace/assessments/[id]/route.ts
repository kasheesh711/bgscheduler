...
import { assessmentDetail } from "@/lib/progress-tests/workspace/data";
import { privateJson, uuidParam, workspaceError } from "@/lib/progress-tests/workspace/http";
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try { const scope = await requireWorkspace(); return privateJson(await assessmentDetail(scope, uuidParam((await context.params).id))); }
