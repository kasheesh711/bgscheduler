...
vi.mock("@/lib/onsite-foot-traffic/data", () => ({
  ...
  readFootTrafficDashboard: vi.fn(),
  readFootTrafficVisitDetails: vi.fn(),
...
const visits = [{ attendanceDate: "2026-03-02", startTime: "10:00", weekStart: "2026-03-02", month: "2026-03", studentFingerprint: "fingerprint", wiseSessionId: "session-1", room: "Focus", subject: "Math", tutor: "Tutor", consumedCredits: 1 }];
const timings = { metadataMs: 1, databaseMs: 2, aggregationMs: 3, totalMs: 6, sessionRows: 1, visitRows: 1 };
...
beforeEach(() => {
  ...
  vi.mocked(readFootTrafficDashboard).mockResolvedValue({ payload, timings } as never);
  vi.mocked(readFootTrafficVisitDetails).mockResolvedValue({
    visits,
    timings: { databaseMs: 2, transformMs: 1, totalMs: 3, visitRows: 1 },
  } as never);
...
describe("onsite foot-traffic API", () => {
  it("requires authentication", async () => {
    ...
    expect(readFootTrafficDashboard).not.toHaveBeenCalled();
  });
  ...
  it("returns only aggregate dashboard fields, never visit details", async () => {
    ...
    expect(response.headers.get("server-timing")).toContain("aggregate;dur=3");
    expect(readFootTrafficVisitDetails).not.toHaveBeenCalled();
  });

  it("rejects unknown CSV grains and keeps visit-detail work exclusive to the visit CSV", async () => {
    const invalid = await getExport(new Request("https://app.test/api/onsite-foot-traffic/export?grain=daily"));
    expect(invalid.status).toBe(400);
    const aggregate = await getExport(new Request("https://app.test/api/onsite-foot-traffic/export?grain=weekly"));
    expect(aggregate.status).toBe(200);
    expect(readFootTrafficVisitDetails).not.toHaveBeenCalled();

    const response = await getExport(new Request("https://app.test/api/onsite-foot-traffic/export?grain=visits"));
    expect(response.status).toBe(200);
    expect(readFootTrafficVisitDetails).toHaveBeenCalledTimes(1);
