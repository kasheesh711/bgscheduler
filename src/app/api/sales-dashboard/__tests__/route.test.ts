...
describe("sales dashboard API routes", () => {
  beforeEach(() => {
    ...
    vi.mocked(importAllSalesSources).mockResolvedValue([{ sourceId: "source-1" }] as never);
    vi.mocked(importRefreshableSalesSources).mockResolvedValue([{ sourceId: "source-2" }] as never);
