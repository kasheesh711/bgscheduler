...
### Known Issues (open)
- **Online/onsite detection heuristic (search/compare only)**: uses `location` field pattern matching (http/online/learn./zoom/meet.google/virtual). Most sessions have venue names like "Think Outside the Box", "Tesla", "Nerd" which don't match → all treated as onsite. Needs a more reliable data source (possibly `sessionType` from Wise or the `isOnlineVariant` flag on the tutor's underlying Wise records). Visual distinction (dashed vs solid border) removed from cards pending reliable detection; modality info still shown in popover. **Resolved for Student Schedule**: the Wise session title prefix is a reliable signal there (`deriveSessionModality`, verified >99.5% against Wise `session_type` in prod) — see [docs/features/student-schedule.md](docs/features/student-schedule.md). Applying the same idea to the tutor snapshot would close this.
...
- **Config**: strict TS, path alias `@/*` → `./src/*`. ESLint 9 flat config (`next/core-web-vitals` + `next/typescript`, no custom rules). `src/lib/env.ts` declares 18 env vars — 7 hard-required, 2 defaulted, 9 optional (3 LINE + `LINE_SCHEDULE_BOT_ADMIN_IDS`, `ENABLE_STUDENT_SCHEDULE_LIVE`, `STUDENT_SCHEDULE_LINK_TTL_DAYS`, `APP_BASE_URL`, `MAINTENANCE_MODE`, `MAINTENANCE_BYPASS_EMAILS`); function `maxDuration` is set per-route (most sync/import routes 800s on Vercel Pro), NOT in `vercel.json`.
...
- **Validation**: Zod schemas as module-scope `const`; always `.safeParse()` (never `.parse()`). Prefer `z.coerce.*` for boundary parsing; Zod also validates external Wise payloads. Env centralized in `src/lib/env.ts` (7 hard-required + 2 defaulted + 9 optional; throws on invalid, logs only `fieldErrors`).
