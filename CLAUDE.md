...
Aoeng's workflow is documented in [docs/operations/aoeng-windows-setup.md](docs/operations/aoeng-windows-setup.md). Use the protected browser preview on `codex/aoeng-preview`; keep credentials off her computer. Kevin owns website enable/disable, repository controls, and preview sharing; see [docs/operations/owner-access-runbook.md](docs/operations/owner-access-runbook.md).
...
```bash
# Deploy to production — merge a checked PR to main; Vercel auto-deploys.
# Ordinary feature changes need the five required CI checks. Protected files in
# .github/CODEOWNERS require Kevin's review. Never bypass branch protections.
# Aoeng uses codex/aoeng-preview; preserve this branch and merge with a merge commit.
...
- **Runtime**: Node.js 24 (`package.json` engines and all five CI jobs), Vercel Serverless Functions pinned to the `sin1` region; 43 route files raise their own `maxDuration` ceiling (no `functions` block in `vercel.json`). TS target ES2017, module `esnext`, `moduleResolution: bundler`. Package manager npm (`package-lock.json`).
...
- **Auth / validation**: `next-auth` 5.0.0-beta.30 (Auth.js v5, Google provider + `admin_users` allowlist; `src/lib/auth-session.ts` validates current admin status/version in server auth and the Node Proxy; `src/lib/auth-edge.ts` decodes its cookie). `zod` ^4.3.6 (99 files) — per-route body schemas plus a centralized env schema at `src/lib/env.ts` that is **dead code**: zero importers anywhere in `src/`/`scripts/`, so it never actually validates or throws at boot.
