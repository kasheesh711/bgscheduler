...
1. Week view placement below day tabs and above the calendar body: PENDING
2. One selected tutor compactness and seven Monday-Sunday segments: PENDING
3. Two selected tutors fixed tutor colors plus non-color density encoding: PENDING
4. Three selected tutors total strip height at or below 120px with no overlap: PENDING
5. Segment click opens existing day drill-down only: PENDING
6. Day drill-down keeps density above `CalendarGrid`: PENDING
7. Tab plus Enter/Space activates a density segment: PENDING
8. Reduced-motion mode has no density-specific animation, pulse, shimmer, delayed reveal, or transition: PENDING
9. VoiceOver segment label includes day, tutor, booked hours, session count, and Open day view: PENDING
10. Copy uses booked hours/session count only, with no availability, capacity, percentage, recommended, or free claim: PENDING
