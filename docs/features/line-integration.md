...
- **A parent-facing send always has a human decision behind it** — an explicit admin decision in the web UI, an explicit `YES` inside LINE, or an allowlisted admin's schedule-bot command in a group the bot has already delivered that student to. That last case is the one exception to "someone typed YES": once `line_group_schedule_sends` holds a row for this (group, student) — *and* the chat has a registered audience, *and* the command carries no `send` verb — a repeat command posts the link straight into the chat with no pending row and no confirmation (`src/lib/line/schedule-bot-group.ts:426-436`). The design rationale in the code is that the command itself is the authorisation, because the destination is a chat that has already received this student — see [GRP-BOT-04](#schedule-bot-gates).
...
The group path re-weights these for a group destination (`src/lib/line/schedule-bot-group.ts:1-35`): the bot must be addressed by typed `/schedule` prefix or a native `isSelf` mention (`GRP-BOT-01`); non-admins get silence (`GRP-BOT-02`); matching is narrowed to a single **exact** bracketed nickname code, because the ranked directory search is far too loose when the result is posted into a family chat (`GRP-BOT-03`, `schedule-bot-command.ts:94-102`); the first time any student appears in a given group a confirm is required, which is the only defence against the right code typed in the wrong family's group (`GRP-BOT-04`, `schedule-bot-group.ts:420-436`); and empty months still refuse (`GRP-BOT-05`). The verified-link gate is deliberately *not* applied in groups — the destination is the chat everyone is already in.
...
- Group audience (`family` / `staff`) selects the message **wording** — the template ternary at `schedule-bot-group.ts:593-607`, described as "only selects the wording" in the `deliver()` JSDoc (`:546-554`). It relaxes no gate, but it is not inert: a registered audience is a precondition of the GRP-BOT-04 straight-through path, so an unregistered chat can never skip the confirm (`:426`); its absence forces the setup prompt in place of the confirm prompt (`:470-478`); and a bare `FAMILY`/`STAFF` reply both registers the chat and authorises the pending student in one step (`:310-315` → `:522-527`).
