...
**Schedule-bot delivery** is a separate loop that never touches the review queue. The link is minted *after* confirmation, not before: command → resolve student (the search also hands back the active snapshot, reused downstream instead of re-resolving) → check the month is non-empty (schedule fetched with `liveSweep: "rescue"` — the live Wise sweep runs only when the snapshot month is empty; the message's session count is otherwise snapshot-based and the minted page stays live) → write a pending row holding only student/month/recipient → confirm → mint → post. In a DM the default path skips the confirm entirely — it mints and replies to the requesting admin — and only the explicit `send` verb resolves a parent. In a group the link goes into the group the command came from, so no identity resolution is involved at all. Both routers log one `[schedule-bot] … outcome=<action> elapsed_ms=NNN` completion line per handled command.
