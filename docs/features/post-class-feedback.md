...
| `GET /api/internal/post-class-feedback/payout-accrual` | Auto-approval sweep, then the accrual pass, then the finalize pass. Scheduled hourly (`33 * * * *`). |

The first two and the payout accrual have a `vercel.json` entry. The three parked reminder/digest routes carry `schedule: null`, `manualOnly: true`, and `dangerous: true` in the cron registry, so Data Health never reports them late but still lets an admin run them deliberately behind a confirmation label (`src/lib/data-health/cron-registry.ts:185`-`247`). The payout accrual keeps `dangerous: true` — the manual Run-now path stays confirm-gated even though it is now scheduled.
...
Required fields are **topics**, **performance** (how the student did), and **improvement** (needs more work on). Homework and due date are stored and displayed but do not affect compliance (`types.ts:1`-`10`). **The content bar is the combined character minimum only** (business decision, 2026-08-29): the three required fields together must total at least **300 raw Unicode code points** (`policy.ts:18`), counted with spaces and line breaks intact, and that count must be carried by at least one field of real (non-placeholder) language — 300 characters of repeated filler still fails (`all_fields_placeholder`). An **empty individual field is informational, never a violation by itself**: per-field ✓/✗ states remain visible in the review UI and per-version rows, but only `combined_characters:N/300` and `all_fields_placeholder` are persisted as an assessment's violating reasons. Normalization, case-folding, and whitespace collapse are used only for placeholder detection and similarity.
...
**Live review + ledger verification (2026-08-30).** The workspace date range defaults to the current 26→25 payout window (prev/next window and calendar-month presets in the header) and background-refreshes every 60 seconds, so the Deductions tab is a live decisions feed: pending items pinned first (longest-waiting on top), decided rows newest-first with a Decided column (actor + Bangkok instant). The ledger hint keys on `payoutLedgerState` — `written` (a live, non-retired ledger row exists), `removed` (its row was written then deliberately removed: netted pair or reinstatement), `none` — and both it and the server-side Process gate (`findWrittenPayoutDeductionLine`) ignore retired lines, so a reinstated deduction correctly reads "Publish required" until its fresh generation row lands. The Payouts tab's **Verify sheet** button (`{action: "verify_sheet"}` on the payout-runs route, finance capability, read-only — one Sheets read via `verifyPayoutSheet` in `payout-sheet-verify.ts`) compares every ledger row and correction against the live sheet: present / missing / amount-changed / expected-absent (retired), lists approved-but-unpublished candidates, and rolls the result up per tutor (expected rows + ฿ vs what the sheet holds) so each tutor's QUERY-view workbook can be checked at a glance. The CLI `report-payout-sheet-reconciliation.ts` is a thin wrapper over the same engine.
...
**Continuous accrual and automated finalize (hourly cron).** `/api/internal/post-class-feedback/payout-accrual` runs an auto-approval/reopen sweep, then the ledger-retirement pass, then the accrual pass, then the finalize pass, every hour at minute 33. Reopen runs before approve so a deduction reopened this tick is not simultaneously treated as a stale approved row (`auto-approval.ts`). A `pending_review` deduction past a grace period (`POST_CLASS_AUTO_APPROVE_GRACE_HOURS`, default 24; production runs an explicit `0` — charge at deadline) on a `live`-enforced, source-`ready` session auto-approves; an `approved`-but-unwritten deduction that loses proof auto-reopens — that half is what keeps `assertPayoutRunPublishable`'s hard `unprovenApprovedDeductions` gate at zero. Neither sweep writes approval logic of its own; both drive `applyPostClassReviewAction`.

**Unattended charging scope (post-INC-260829 re-enable).** The whole unattended pipeline keys on `POST_CLASS_AUTO_APPROVE_ENABLED` being the exact string `true`: the approve sweep, the payout-candidate carve-out that admits exactly `system:post-class-auto-approve` as a decision-maker (`selectPayoutRunCandidates`, `payout-repository.ts` — every other `system:*` actor stays excluded), and the retirement pass. Scope is bounded twice (`autoChargeLowerBoundUtc`, `auto-approval.ts`): never before `PAYOUT_AUTO_CHARGE_FLOOR_BANGKOK` (2026-08-26 — the INC-260829-era backlog and the settled 2026-08 ledger stay human decisions), and never before the last-ended payout window's start, so months-late flags always fall back to the review UI. Flipping the flag off instantly restores human-only approvals; `POST_CLASS_PAYOUT_WRITES_ENABLED` remains the independent kill switch for all sheet writes.

**Ledger retirement (auto-un-charge, `payout-retirement.ts`).** Because charging is instant, evidence that arrives later and clears a violation must take the written row back OFF the ledger — by marker-located row deletion plus `retired_at`, never by netting a +฿100 correction. Each tick retires live written lines inside the automation scope whose deduction was waived, whose session became ineligible, or whose latest assessment on a source-ready session no longer finds an objective violation; deletion is proven by readback before any line is retired, pending corrections on a retired line are superseded, and a hand-edited sheet amount is left untouched for Verify sheet to flag. Retiring first is what keeps the no-netting invariant — a subsequent waive sees no live written line, so `createPayoutAdjustment` is never reached, and the reopen/ineligible sweeps finish the deduction's lifecycle in the same tick. The pass stands down for a tick rather than fight a live publish lease, and a generation-2 line (hash-derived marker) re-charges cleanly if evidence later re-proves the violation.

The accrual pass publishes with `mode: "accrual"`, which skips the window-ended guard, skips the CSV/Drive leg entirely, and forces `partial` so it can never mint `published` while the window is open (`payout-run.ts:697`, `:713`, `:934`, `:939`). The finalize pass targets the **oldest** un-finalized ended run at or after the automation floor, so a window that fails to finalize keeps being retried however many months pass; only when none exists does it fall back to the window anchored to today's own calendar month — and that fallback is the only branch that can create a run row, which is what stops it minting an empty `published` run for a window the system never observed. Both branches see the clock shifted back by a **3-Bangkok-day settlement lag** (`PAYOUT_SETTLEMENT_LAG_BANGKOK_DAYS`, `payout-accrual.ts`): the feedback deadline is 23:59:59 two days after the class date, so the last classes of a window can still produce brand-new proven violations through the 27th, and finalizing on the 26th would strand them as approved-but-unwritten on a `published` run.

A window still un-finalized once its anchor month has passed — or with no run row at all — is surfaced by the cron watchdog as a synthetic `post_class_payout_window` entry (`payout-window-health.ts:49`-`86`). The check is gated on the accrual cron actually having a schedule (`payout-window-health.ts:101`), so it armed itself the moment the cron was scheduled.
...
2. **Are payout writes enabled in production?** The accrual/finalize cron is now scheduled hourly (`33 * * * *` in `vercel.json`), but `POST_CLASS_PAYOUT_WRITES_ENABLED` must still be the exact string `true` and the full `POST_CLASS_PAYOUT_*` target set must be present for a tick to write anything. Whether the production environment carries those values — and whether the tutor-workbook fleet has been cut over to the composite tab — is an operational fact, not a code fact.
...
7. ~~**`POST_CLASS_AUTO_APPROVE_GRACE_HOURS` is read with a bare `Number(... ?? 24)`**~~ Resolved: `resolveAutoApproveGraceHours` (`auto-approval.ts`) now trims the value and falls back to 24 for blank, non-numeric, or negative input; an explicit `0` remains a deliberate immediate-approval mode.
