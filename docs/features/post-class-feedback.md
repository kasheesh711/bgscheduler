...
**Acknowledgeable** — readability and resolvability, both required at 80%, plus a minimum sample of 20 recent eligible sessions. Below that an access manager may proceed only by echoing the exact server-computed count with a reason, both recorded in `post_class_config_audit_log`. This mirrors `assertPayoutRunPublishable`, which gates the actual movement of money the same way.

Both rates are **scoped to the trailing 4-day collector window**, not to a run's whole candidate pool. The feedback-event and incomplete-recheck lanes carry no lower date bound, so a run routinely observes months-old backlog: in production one run saw nineteen historical sessions and one current one, and the unscoped rate read 50% while the period actually being enforced was at 100%. Sessions ending before the activation instant are assessed but can never produce a deduction, so judging the pipeline on them measures nothing that matters. Resolvability is therefore read from persisted state (`loadPostClassRecentSessionReadiness` — eligible sessions in the window that reached `ready`), while readability stays a run measure over the rolling lane (`rollingSavedCount / rollingSelectedCount`), because a session whose detail fetch failed never gets a row and would be invisible to a state query. A zero denominator on either side **blocks**; it is not treated as a pass.
