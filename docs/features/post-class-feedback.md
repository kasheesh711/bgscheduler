...
The scheduled collector runs at `12,42 * * * *` (UTC cron expression, every 30 minutes). A normal run:
...
The feature owns 24 snapshot-independent tables. Exact definitions and constraints are in `src/lib/db/schema.ts` and migration `drizzle/0048_post_class_feedback.sql`.
...
After deploying the code and applying `0048_post_class_feedback.sql`, Kevin should complete these steps in the website's Settings view:
...
_Verified against HEAD + uncommitted WIP on 2026-07-26._
