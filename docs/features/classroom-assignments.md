...
| Area | Path | Role |
|---|---|---|
| Domain library | `src/lib/classrooms/` | Room catalog and tutor rule tables, the assignment engine, continuity reconciliation and stable teacher room profiles, run/publish persistence, tutor + admin email builders, floor-plan geometry, visualization helpers |
| Admin API | `src/app/api/class-assignments/`, `src/app/api/classrooms/` | 13 endpoints: 12 session-gated (11 under `/api/class-assignments` + `GET /api/classrooms/rooms`) and 1 public SVG (`GET /api/classrooms/floor-plan-map`, which never calls `auth()` — `floor-plan-map/route.ts:1-16`) |
| Cron API | `src/app/api/internal/class-assignments/` | `morning` automation and `admin-email` digest |
| UI | `src/app/(app)/class-assignments/page.tsx`, `src/components/class-assignments/` | One client workspace with five tabs, two dialogs and an authenticated A4 print surface |
...
| `POST /api/class-assignments/run` | Generate a reconciled assignment run for a date, optionally discarding prior overrides. Refuses to run from a stale snapshot (see [Freshness](#freshness-and-wise-coupling-srclibclassroomsdatats)). Never writes to Wise. |
...
| `GET /api/internal/class-assignments/morning` | Daily 17:00 Bangkok cron: sync → assign seven days starting tomorrow → selective publish; defer email. |
| `GET /api/internal/class-assignments/admin-email` | Daily 19:00–19:46 Bangkok delivery window: tomorrow's teacher schedules, then the admin summary. |
...
| Tutor Schedule | shared schedule projection | Canonical teacher checklist (first four pre-selected), usual rooms, chronological classes, short-gap room changes and exceptions. |
| Usual rooms | `usual-rooms-panel.tsx` | Persisted sets of up to three rooms, with revision-checked admin edits and unavailable-room warnings. |
...
flowchart TD
  ...
  subgraph Cron["Cron 0 10 * * * UTC (17:00 Bangkok)"]
    ...
    C2 --> C3["for each of 7 days from tomorrow:<br/>runIncrementalClassroomAssignment<br/>(reconcile minimal_moves)"]
    C3 --> C4["selectAutomationPublishTargetRowIds<br/>→ publishClassroomAssignmentRun"]
    C4 --> C5["Saved plans ready for evening delivery"]
  end
  subgraph Digest["Cron 0,16,31,46 12 * * * UTC (19:00–19:46 Bangkok)"]
    D1["Tomorrow: teacher schedules, then admin summary"] --> D2{"run exists & no publish pending?"}
    D2 -- "yes" --> D3["digest to unsent admin recipients<br/>ACTION REQUIRED if blockers remain"]
    D2 -- "no, before 19:46" --> D4["pending — retry next tick"]
    D2 -- "no, at/after 19:46" --> D5["'ACTION REQUIRED' failure digest"]
  end
```

**Manual generation and automation** both call `runIncrementalClassroomAssignment`. They initialize missing usual-room profiles, merge the fresh snapshot with already-started saved classes, read live Wise occupancy, check date-wide teacher delivery history and reconcile the previous saved plan. A transaction inserts the new run and all rows together. `forceReassign` discards explicit overrides but does not bypass notified/started preference protection.

**Override edits** use the same reconciliation and continuity optimizer against the selected run and fresh external Wise occupancy. The explicit override can change its requested row. Other notified/started classes remain protected from preference-only optimization. Updates are atomic, reject a concurrently changed run and preserve unchanged rows' publish state.
...
**Next-day preparation** (`daily-automation.ts` → `morning-automation.ts`). At **17:00 Bangkok**, prepare and publish the seven dates beginning tomorrow. The existing freshness guard captures one promoted snapshot and shares any running Wise sync, with up to ten minutes of waiting. Teacher emails are deferred. Unresolved rooms/publishing remain visible; completion is not guaranteed at the exact trigger minute.

**Evening delivery** (`daily-automation.ts` → `schedule-email.ts`, then `admin-schedule-email.ts`). At **19:00 Bangkok**, email tomorrow's saved teacher schedules, then the admin summary. Retry failed/blocked delivery at 19:16, 19:31 and 19:46 while preserving successful recipients. Require a run created since today's 17:00 preparation; yesterday's provisional horizon is not sufficient. Onsite schedules must have successful publishing. Missing preparation or pending publishing delays the admin summary until the final 19:46 tick, when an `ACTION REQUIRED` message goes out. Tutor delivery errors still reach the admin summary.

The historical route names and audit keys remain; Data Health labels show their current purposes. Manual Data Health actions also target tomorrow. Explicit room corrections remain possible; there is no new irreversible daily lock.

## Stable usual rooms and continuity (continuity-v1)

`room-profile-planner.ts` chooses up to three familiar rooms per resolved `canonicalKey`, merging online/onsite accounts. It keeps existing preferred classrooms first, uses the upcoming 28-day schedule to rank compatible teaching time and competing demand, then the latest daily assignments from the preceding 28 days and deterministic room ordering. Gift keeps Joy alone. Only missing profiles initialize; routine generation never replaces persisted sets. Admin edits use room UUIDs and compare-and-swap revisions. Sets are shared preferences, not reservations; inactive rooms remain visible until corrected. The print/profile GET paths never initialize data.

The chronological allocator and 20,000-node feasibility repair build a valid starting plan. `continuity-optimizer.ts` then considers complete consecutive-class chains, adjacent pairs, single placements and bounded displacement/swaps, even when every class is assigned. The additional budget is 10,000 nodes at depth four. Plans compare, in order: unassigned classes, 0–15-minute room changes, assignments outside usual rooms, changes to saved assignments, distinct teacher rooms, then room preference/fit. Capacity, TV, modality, external occupancy, overrides, Gift and Kevin/Mek/Ras rules stay enforced. The separate online-needs-center window remains strictly under 60 minutes.

Future Wise-published rooms may improve before teachers receive their daily schedules. Successful delivery is checked by canonical identity and date across every run. Started classes and notified teachers are protected against preference-only changes; real constraint changes and explicit overrides still require operational attention. Teacher delivery starts at 19:00 Bangkok on the preceding day. Email retries skip already-delivered teachers across run/snapshot rotation; an all-delivered rerun creates no new email attempt. Usual rooms and exceptions appear in the existing email.

Each run snapshots `algorithmVersion`, `roomPolicies` (including revisions), and `quality` in `changeSummary`. Quality includes consecutive pairs, room changes, profiled assignments, outside-usual assignments, coverage, distinct rooms and continuity search usage/exhaustion. Reaching a bound does not establish that a remaining move is unavoidable. `CLASSROOM_CONTINUITY_ENABLED=false` restores feasibility repair without the new continuity/profile optimization; profiles and saved print history remain intact.

## Daily noticeboard sheets

**By tutor / By room** chooses the print grouping (tutors by default). **Print day** opens the selected saved run, including an empty run. **Print seven days** resolves the latest saved run for each of seven dates beginning at the selected date, reports missing dates and opens an explicit run-ID list at `/class-assignments/report?runIds=…&view=tutors|rooms`. Existing links default to tutors. The report is authenticated and read-only; it neither generates assignments nor publishes them.

`schedule-projection.ts` supplies the saved scheduling facts. `print-roster.ts` refreshes exact Wise session student lists when the report opens and again through `GET /api/class-assignments/print-report` before printing. One strict future-session sweep serves the entire report. Started/past, absent and incomplete sessions use individual session-detail reads; a single student-directory fetch resolves bare IDs if needed. Membership is never inferred from titles, package names, participants or general course membership. IDs are deduplicated before name resolution; different students with identical names remain separate. Only names and print-relevant status information leave the server, with no student IDs, contact details, subjects or class titles.

For detail reads, an exact live session match supplies the current class ID when an older saved row lacks it or the class has changed. The saved class ID remains the fallback when the future sweep omits a session. A changed class still becomes a schedule exception; a class ID is only an endpoint locator, never evidence of student attendance.

Tutor sheets retain two alphabetical columns and list students below each time/room. Room sheets contain chronological Time · Tutor · Students rows, one active room per A4 landscape page in catalog order. Empty rooms say “No classes scheduled.” Long names wrap and large groups continue on labelled pages without reducing the 11pt minimum timetable text. Both layouts preserve BeGifted's official logo, orange `#FF7518`, blue `#126DCE`, ink `#16203A`, Cormorant Garamond headings and Sarabun body/Thai text. Font-measured pagination reruns after roster refresh. Every page repeats its date, Bangkok generation/roster-check times, content revision and page number; student names and exceptions participate in the revision.

Saved times and room decisions remain intact. Session verification and started-session roster refresh convert stored Bangkok wall-clock timestamps to UTC instants using the publisher's shared converter (`src/lib/classrooms/timestamps.ts`). Comparing the raw database Dates with Wise instants would falsely mark unchanged sessions as rescheduled and remove them from both tutor and room sheets. Cancelled or changed Wise sessions appear only in a labelled exceptions section directing staff to regenerate assignments. Remote sessions stay on tutor sheets; room-mode exceptions also include unassigned/unavailable rooms. Missing names, unverifiable rosters, capacity overflow and assignment/publishing warnings mark the report **DRAFT**. A failed Wise refresh displays a retry action and blocks printing stale rosters. Use the report's **Print / Save PDF** button: it refreshes, repaginates and then opens the print dialog. Direct browser printing instead shows a reminder to use that button. Assignment edits during report loading reject the read and require a reload. Printing adds no database records and needs no migration.

Validation and rollout evidence: [2026-09-07 classroom stability](../operations/classroom-stability-2026-09-07.md).
...
### Reconciliation for manual and unattended runs (`src/lib/classrooms/reconciliation.ts`)
...
- Same fingerprint as the previous run → an initial `carried` candidate before continuity optimization for successful/remote/review rows; unchanged `no_room` rows are retried. For carried rows, the previous room, status, warnings, trace **and publish state** are copied verbatim (`:190-213`). Otherwise the change is `added`, `changed` (same times) or `rescheduled` (`:177-188`), and the row is re-assigned with publish state reset (`:215-229`). Future sessions missing from the snapshot produce a `canceled` event; started saved classes are retained by the data layer (`:329-342`).
...
- A tutor with any `needs_review`, `no_room`, or failed/unpublished onsite row that day is `blocked` (`unfinalized_rows`, `:515-535`); a run is `sendable` only with no hard blockers (Apps Script env config present, at least one row) and ≥ 1 ready tutor (`:460-497, 584`).
- Teacher messages use the shared **BeGifted v3 email renderer** (`src/lib/teacher-emails/`). A single-column chronological agenda includes time, student/class, subject, mode and room; usual rooms, outside-usual-room exceptions and room-change notices remain visible. HTML and plain text come from the same prepared content. Subjects include the Bangkok assignment date with its weekday and named month.
- **View school map** opens the existing public floor-plan SVG with the assigned rooms pipe-joined and the existing cache version; the image is no longer embedded. Entirely remote emails omit the map action. The existing `roomSteps`, `blocks` and `mapImageUrl` preview fields are retained. URL precedence remains `SCHEDULE_EMAIL_PUBLIC_BASE_URL` → `VERCEL_PROJECT_PRODUCTION_URL` → `VERCEL_URL` → `https://bgscheduler.vercel.app`.
- The schedule dialog displays the **actual delivery HTML** in a sandboxed iframe, with subject, plain-text and phone-width controls. Recipient selection and sending behavior are unchanged. The only new public static asset is `/brand/email/v3/logo-horizontal.png`; it remains accessible during maintenance, without exposing other brand files.
- Run `npm run emails:preview` to generate the synthetic, offline-capable gallery at `output/teacher-emails/index.html`. It includes schedules, progress-test heads-ups, feedback reminders and the feedback delivery test, without accessing the database or sending mail. Browser verification does not establish native email-client rendering.
- **Transport is a Google Apps Script relay** (`POST` JSON with a shared secret; `:597-635`), configured by `SCHEDULE_EMAIL_APPS_SCRIPT_URL/_SECRET` with a `SCHEDULE_EMAIL_BACKUP_*` pair (`:288-304`). Sender name defaults to `BeGifted`; reply-to defaults to a hard-coded personal address (`:606-607`). Per-recipient idempotency key: `classroom-schedule:<assignmentDate>:<canonicalKey>:<16-hex content hash>` (`:637-650`).
...
- `mode: failed_only` skips tutors with a `sent` record across all runs for the same date and canonical identity and retains blocked recipients in the delivery ledger. The evening delivery therefore records tutors such as Shop even when an unresolved room prevents sending.
...
- The admin summary uses the 19:46 Bangkok final retry deadline, matching the last `0,16,31,46 12 * * *` cron tick.

### Recovery preview and failure reporting

`morning-automation.ts` returns compact `noRoomCount`, `needsReviewCount`, `failedPublishCount`, `failedEmailCount`, `blockedEmailCount`, `unmanagedWiseSessionCount` and `errorSummary` fields. Incomplete runs return `ok: false`/HTTP 500 while retaining completed assignments, publish results and delivery history. Live sessions missing from the normalized snapshot are persisted by ID/count in run metadata and select an actionable sync/identity-review alert. Partial admin delivery also returns HTTP 500. Historical route paths are retained; preparation and delivery use the evening schedules above.
...
A private checkpoint saves Saturday and Sunday allocations on Wednesday at 09:00 Bangkok, with retries at 09:16 and 09:31. Fresh verified Wise data and the minimum-switch optimizer produce actual-modality allocations plus separate conditional conversion recommendations. Each checkpoint/date can save only one run; retries reuse it if source constraints are unchanged. Wednesday always emails the configured private recipient, including zero-switch results. Thursday/Friday checkpoints are read-only and repeat warnings while problems remain, followed by one verified recovery notice. Daily publishing and tutor delivery remain separate.

The version-2 weekend report carries each date's saved run ID, source timestamps, overflow plan and fresh Wise publication state. Class Assignments and the private email show before/after rooms, existing-online releases, student switch evidence and conditional accommodation. A failed date remains unverified while the other date can finish. Existing-online releases appear in operational schedules; proposed conversions never alter those schedules or published modalities.
...
## Minimum-switch overflow planning

Assignment generation invokes the server-side HiGHS 1.15.3 integer optimizer only when the ordinary allocator still has `no_room` lessons. It considers whole-lesson intervals, physical room aliases, capacity, equipment and confirmed reservations using the allocator's shared compatibility rules. Explicit overrides, started/notified lessons, uncertain source data and unsupported modalities are protected. Days without overflow retain the normal allocation path.

The first solve permits only classroom changes and relocation of existing online lessons. Online overflow relocations use a dedicated online room when possible and otherwise **Teach elsewhere — classroom released**. The persisted `overflow_release_room` instruction overrides the adjacent-onsite retention rule and appears in staff readiness findings and tutor schedules. It is cleared on a changed lesson, roster, tutor, modality or explicit override. It never licenses an onsite lesson to become remote.

If necessary, a second solve proposes conversions of verified one-to-one onsite lessons. Both allocated and unallocated lessons participate. The objective order is complete accommodation, number of lesson conversions, summed history ranks, dedicated-online-room placement, other room changes, then stable room order. Group lessons are not conversion candidates. Dedicated-room preference is a placement requirement within an equally ranked conversion set; a booth never preserves an onsite classroom requirement when none is available.

The total optimization budget is 30 seconds, including at most 10 seconds for the actual-modality phase. Results distinguish `minimum_proven`, `best_found`, `no_complete_solution` and `unverified`. Only an optimal integer phase proves a positive minimum or infeasibility; any verified zero-switch complete plan trivially proves zero. A feasible timeout result is labelled best found, and history/room ranking completeness is reported separately. The lower bound, when available, concerns the number of switches in a complete plan.

Actual-modality improvements are saved in assignment rows. Hypothetical conversions and their full-day predicted assignments live separately in versioned `changeSummary.overflowPlan`, also exposed in assignment responses. Fresh matching snapshot **and** live Wise ONLINE evidence is required to activate a proposed conversion's room-release instruction. Proposed actions never rewrite actual modality, publish to Wise, send notifications or clear unresolved readiness. Manual overrides discard the old recommendation because its constraints have changed.

Student evidence uses stable Wise student/session IDs within a 180-day lookback. Durable observations are recorded from successful existing Wise, past-session and Credit Control syncs; there is no new scheduled job. A verified switch requires ordered onsite/online observations of the same occurrence, an unchanged authoritative roster, no cancellation, and final online attendance. Duplicate observations count once per lesson. Credit Control attendance joined to the exact past-session ID and times is explicitly fallback evidence: it cannot create a verified transition. Creation events and names are not used.

Candidates rank by verified switches, online attendance fallback, known onsite-only history, then unknown history. Within tiers the score is successes / (observed opportunities + 3), followed by evidence volume and recency. The UI shows actual counts and observed dates; a 180-day lookback does not imply 180 days of complete coverage. Missing history does not fabricate a switching rate.

See [overflow planner rollout and validation](../operations/classroom-overflow-planner.md) for migration, bootstrap and deployed WASM verification.
