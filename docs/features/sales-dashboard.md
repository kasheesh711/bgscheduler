...
- `GET /api/sales-dashboard/actions` — return the GM action cockpit for the selected date range (`src/app/api/sales-dashboard/actions/route.ts`).
- `GET /api/sales-dashboard/dimensions` — return tabbed-workspace aggregates for reps/programs/packages/students (`src/app/api/sales-dashboard/dimensions/route.ts`).
- `GET /api/sales-dashboard/transactions` — return paginated slim transaction rows for drill-down tables (`src/app/api/sales-dashboard/transactions/route.ts`).
- `GET /api/sales-dashboard/transactions/export` — export all matching slim transaction rows as CSV; same filters as the transactions endpoint, no pagination (`src/app/api/sales-dashboard/transactions/export/route.ts`).
