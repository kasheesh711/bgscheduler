...
- **`docs/reference/database/erd-credit-control.md` and `docs/reference/api/credit-control.md` do not exist yet.** This doc links to the conventional canonical paths per the canonical-home rule. The `docs/reference/` tree itself is not empty (it currently holds `crons.md` and `env.md`), but the `database/` and `api/` subdirectories are absent, so these two reference files need to be authored for the links to resolve.
