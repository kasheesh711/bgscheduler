...
**Aoeng: start with [your Windows + Claude Code guide](./operations/aoeng-windows-setup.md).** It walks through setup, browser previews, and publishing without a local database. Kevin's [owner runbook](./operations/owner-access-runbook.md) covers permission changes and recovery.
...
- **[`operations/`](#operations)** — collaborator setup, owner access, deployment/runbook, auth model, observability, maintenance, and dated operational records.
...
| [Onsite Foot Traffic](./features/onsite-foot-traffic.md) | stable | De-identified Wise PAST-session attendance: student-visits, unique students, classes, weekly/monthly and room/weekday patterns, CSVs, and a standalone HTML/PDF analytics pack. Production collection is active from March 2026. |
...
| [Leave Requests](./features/leave-requests.md) | stable | Daily roster-owned teacher/date assignments with family notification and manual Wise cancellation checklists; automatically interprets source submissions with Luna low. |
...
| Feature | Badge | Summary |
|---|---|---|
| [Progress Tests](./features/progress-tests.md) | stable | Tutor-owned one-to-one preparation, grading, native Wise publication and upload-to-PDF paper preparation, with fixed eight-class cadence. |
...
| [aoeng-windows-setup.md](./operations/aoeng-windows-setup.md) | Aoeng's step-by-step Windows + Claude Code/GSD guide: own accounts, browser preview, checks, pull requests, and publishing. |
| [owner-access-runbook.md](./operations/owner-access-runbook.md) | Kevin's website enable/disable controls, independent GitHub/preview revocation, protected publishing rules, preview isolation, and recovery. |
| [auth-and-access.md](./operations/auth-and-access.md) | Who may sign in and what they may reach: Google-only Auth.js v5, fresh admin status/version checks in Node Proxy and server auth, owner controls, page scoping, and per-feature grants. |
...
| [maintenance-mode.md](./operations/maintenance-mode.md) | The `MAINTENANCE_MODE` kill switch and its `MAINTENANCE_BYPASS_EMAILS` allowlist: what the request gate covers, its cron/public-parent exemptions, and the flip-and-redeploy procedure. |
