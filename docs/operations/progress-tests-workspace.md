...
## Preparation publication — 2026-09-14

The preparation action now requests native publication of the exact approved `pt_paper_artifacts.kind = paper` PDF, independently of final-result approval. The marking key is never an input. Each assessment has an ordered preparation-publication history, immutable artifact/hash bindings and resumable upload/removal checkpoints. Existing prepared assessments are not backfilled. Publishing pause applies to preparation and results.

Replacement attaches and verifies the new paper before withdrawing the previous app-managed resource. Removal clears this assessment's selection and informed checkbox, preserves topics/library/history, and withdraws only its recorded preparation resource. Submitted assessments retain their paper. Concurrent operations and uncertain attachment outcomes require reconciliation; native create POSTs are never blindly retried.

The Wise client release `1789131705` exposes `POST /teacher/deleteResourceInBulk/` with `{classId, sectionId, entityType: "resource", resourceIds: [resourceId]}`. The application verifies the course, section, exact recorded resource and hash before deletion, and confirms its absence afterward. This observed native contract must pass the labelled synthetic upload/replace/remove operator check before release. No session, assessment, unrelated resource, or section deletion is permitted.

At **2026-09-14T03:03:19Z**, `scripts/verify-progress-preparation-publication.ts --live-wise` passed against the established validation course/student/section using a disposable localhost database and the separate validation Blob store. Exact hashes matched both labelled synthetic PDFs. Upload resource `6aa763ede5e946e902e9e91b` was withdrawn only after replacement `6aa763f2b42540aba072e2ce` verified; removal then withdrew the replacement. All pre-existing resources remained, all three immutable operation records were retained, and the local count stayed at eight. No paid AI calls or production assessment writes occurred. Evidence is in ignored `output/progress-tests-verification/preparation-release/native-publication.json`. The script resumes recorded failures without blindly reattaching.

Layout PR #72 is live at main commit `4209ff39730daf133c51be66d5b35dd58ae7bf3b`. This upgrade does not call activation or alter attendance counters. Migration 0087 must be applied before deploying the preparation-publication server.

Migration 0087 was applied before deployment with every existing migration hash checked. The launch timestamp remained `2026-09-13T11:36:37.162Z`; all 563 series, two qualifying classes, seven paper versions, 563 assessments, settings revision 3, and existing review/publication fingerprints matched before and after. The new ledger started empty. Private preflight and migration evidence live alongside the native verification record.

Release checks passed: 4,997 unit tests, 48 Progress Tests integration tests, TypeScript, scoped ESLint, production build and the 255-route release guard. Browser checks passed at 1440 × 900 and 390 × 844 in light/dark themes: the preparation panel begins 205 px below global navigation, replacement preserves topics and resets informed confirmation, paused upload survives reopening, removal preserves both library choices and topics, and the mobile Details/PDF switch has no horizontal page overflow. No browser warnings or errors were observed.

Paused preparation publication also preserves submission/grading availability. An integration test submits during the pause, verifies that replacement/removal are then locked, and completes the original upload after publishing resumes.
