...
| `500` | Either `CRON_SECRET` is unset server-side (`Server misconfigured`) **or** the job failed. Wise sync uses `outcome: "failed"` when no usable promotion was confirmed, with `errorSummary` or `error`; HTTP 500 is no longer caused solely by teacher review issues after promotion. | [`cron-auth.ts:22-24`](../../src/lib/internal/cron-auth.ts), [`run-wise-sync.ts`](../../src/lib/sync/run-wise-sync.ts) |
