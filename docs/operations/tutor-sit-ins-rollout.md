...
- The complete release sequence passed on Node 24: both TypeScript checks, **5,135 unit tests across 452 files**, optimized production build, whitespace validation and the **278-route** guard. Local Vitest concurrency was bounded to four workers. Targeted ESLint passed without warnings.
- **44 focused Postgres integration tests** passed against a disposable local database. They cover confirmation without Calendar or enabled delivery; live Wise conflicts, cancellations and notice; concurrent confirmation; connection after confirmation; delayed and missed invitations; uncertain-write retry recovery for both providers; Calendar changes leaving bookings intact; report submission and family acknowledgements while delivery is pending; and the exact migration's preservation of uncertain legacy event writes.
- Desktop 1440×1000 and mobile 390×844 checks passed using the real components with synthetic local APIs: Wise-ready confirmation with delivery paused, separate queued delivery status, all ten rubric ratings and submission, destination-only Calendar settings, staff acknowledgements, dark mode and overflow. These checks do not establish production OAuth consent or live provider delivery.
