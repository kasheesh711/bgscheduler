...
All routes live under `src/app/api/**/route.ts` (Next.js App Router). The tables below list **143 endpoint paths**, including 16 Post-Class Feedback routes (12 capability-gated admin paths and four cron-only paths).
...
Sales reporting: payload load, action feed, dimensions, transaction drills/CSV export, source CRUD & seeding, Google-Sheets imports, import-run history, and projection source/import.

| Method | Path | Group | Auth | Brief purpose |
|---|---|---|---|---|
| GET | `/api/sales-dashboard` | sales-dashboard | admin | Load the sales-dashboard payload |
| GET | `/api/sales-dashboard/actions` | sales-dashboard | admin | Load the GM action cockpit for a date range |
| GET | `/api/sales-dashboard/dimensions` | sales-dashboard | admin | Load tabbed-workspace dimension aggregates |
| GET | `/api/sales-dashboard/transactions` | sales-dashboard | admin | Load paginated slim transaction rows |
| GET | `/api/sales-dashboard/transactions/export` | sales-dashboard | admin | Export slim transaction rows as CSV |
