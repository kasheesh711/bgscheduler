...
> **Scope.** This page documents **25 endpoints** across 14 route files. Eight further `/api/internal/*` endpoints exist in the tree and are documented with their owning features: `GET`/`POST /api/internal/sync-sales-dashboard` ([sales-dashboard.md](sales-dashboard.md)), `GET /api/internal/sync-wise-activity` ([wise-activity.md](wise-activity.md)), `POST /api/internal/sync-room-utilization` ([room-capacity.md](room-capacity.md)), `GET /api/internal/sync-onsite-foot-traffic` ([onsite-foot-traffic.md](onsite-foot-traffic.md)), `GET /api/internal/sync-unearned-revenue` ([unearned-revenue.md](unearned-revenue.md)), and `GET /api/internal/class-assignments/morning` + `GET /api/internal/class-assignments/admin-email` ([classrooms-and-assignments.md](classrooms-and-assignments.md)). Thirty-three `/api/internal/*` endpoints exist in total.
...
Nineteen paths are registered in [`vercel.json`](../../../vercel.json), and every schedule is pinned by a regression test ([`vercel-crons.test.ts`](../../../src/__tests__/vercel-crons.test.ts)). The in-app registry [`cron-registry.ts`](../../../src/lib/data-health/cron-registry.ts) declares **24** jobs — the 19 scheduled ones plus five `manualOnly: true` entries with no `vercel.json` line. Of the endpoints on this page, four are `manualOnly`: the three parked post-class jobs (`admin-digest`, `reminder-day-after`, `reminder-deadline`) and `line-backlog-recovery`.
