...
| GET `/api/room/availability?date=YYYY-MM-DD` | Bearer capability | Optional date defaults to today; only today/tomorrow allowed. Returns `date`, `todayDate`, `tomorrowDate`, `nowMinute`, `tutorName`, `fresh`, `availabilityStatus` (`ready`, `stale`, `updating`, `no_catalog`), `uncertain` intervals, `checkedAt`, `writesEnabled`, `classes`, `rooms`, and owned date-labelled `reservations` |
| POST `/api/room/reservations` | Bearer capability | Strict JSON: `roomId` UUID, `date` YYYY-MM-DD, integer `startMinute`, `endMinute`, optional `immediate` (today only), UUID `idempotencyKey`; returns `{reservation}` |
...
| GET `/api/class-assignments/reservations?date=YYYY-MM-DD` | Admin | Date, room occupancy, reservations, availability status, unresolved intervals, last check and collector error |
| PATCH `/api/class-assignments/reservations` | Admin | `{id}` cancels a reservation with admin audit and private notification |
| GET `/api/internal/room-booking` | Cron secret | Occupancy refresh for both dates, returning `ok` and per-date `dates` results (partial failure is HTTP 500), and durable retries; `maxDuration = 300` |

Availability intervals use integer Bangkok minutes and half-open `[startMinute,endMinute)` boundaries. Rooms include ID/name, capacity, TV/category, occupancy intervals labelled `class` or `reservation`, and proven free intervals. The tutor view excludes raw Wise IDs and other tutor identifiers. Classes include the tutor’s times, room or Remote/Room TBC, `roomSource` (`wise`, `classroom_plan`, or null), and a differing planned room when applicable. A `classroom_plan` source identifies an online class with a blank Wise location and a current matching assignment; it does not claim a Wise publication.

Booking failures include `BOOKING_DISABLED`, `ROOMS_UPDATING`, `STALE_ROOMS`, `ROOM_UNAVAILABLE`, `ROOM_CONFLICT`, `UNRESOLVED_ROOMS`, `TUTOR_CONFLICT`, and `PAST_TIME` (409); invalid time/date/interval inputs use 400 (`INVALID_DATE` for dates outside today/tomorrow and `INVALID_IMMEDIATE` for `now` on tomorrow). Bad or expired grants use 401, and unavailable tutor approval uses 403. Foreign cancellation IDs return 404. Unexpected service failures return a generic 500 without tokens or backend details.
...
LINE accepts an optional `today`, `tomorrow`, or absolute `YYYY-MM-DD` prefix before room-view/free/book commands. Date selection, time pickers, room cards, pagination, and confirmations store absolute dates so midnight never shifts an existing request. `/room bookings` lists upcoming reservations on both dates. A conflicting confirmation offers refreshed alternatives and requires a new confirmation.
