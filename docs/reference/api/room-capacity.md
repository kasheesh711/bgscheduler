...
**Scheduling — manual only.** This endpoint is **not** registered in [`vercel.json`](../../../vercel.json) (the cron list does not include `sync-room-utilization`). Vercel Cron invokes endpoints via `GET`, and this handler exports only `POST`, so it is not auto-scheduled. It is triggered by:
