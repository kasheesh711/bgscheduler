...
**Status:** Stable. **Canonical home:** this page owns the mechanical request/response contracts for the six Wise Activity endpoints. Feature meaning — what the audit log and reconciliation workbench are *for*, and why they stay independent of the tutor-snapshot sync — lives in [`features/wise-activity-audit.md`](../../features/wise-activity-audit.md). The cron ingest endpoint's scheduling/stagger mechanics also appear in [`reference/crons.md`](../crons.md) (this page does not restate them).

The Wise Activity surface is a **read-only audit log** of operational and financial events from the Wise platform, plus a **package-sales reconciliation** view. It maintains its own append-only event store (`wise_activity_events`) and sync-run ledger (`wise_activity_sync_runs`, both defined at [`schema.ts:245-298`](../../../src/lib/db/schema.ts)), and never writes back to Wise or participates in tutor-snapshot promotion (see the feature doc).

All six handlers are Next.js App Router route handlers. Each wraps its work in `try/catch` and returns `{ "error": string }` with an appropriate status on failure; the fallback message differs per route (cited below). **None of the six use a Zod schema** — query params are parsed/clamped by local helpers plus a `^\d{4}-\d{2}-\d{2}$` date regex, and POST bodies are read with a `try/catch` that falls back to `{}` and coerced field-by-field.
...
| 6 | `GET /api/internal/sync-wise-activity` | `CRON_SECRET` bearer | [`sync-wise-activity/route.ts:12`](../../../src/app/api/internal/sync-wise-activity/route.ts) |

### Authentication model

Two distinct gates apply, set by `isPublicRoute` in [`middleware.ts:4-15`](../../../src/middleware.ts):

- **Endpoints 1–5** live under `/api/wise-activity/` — **not** a public prefix, so the middleware redirects unauthenticated browser requests to `/login` ([`middleware.ts:40-49`](../../../src/middleware.ts)). Each handler additionally calls `await auth()` and returns **HTTP 401 `{ "error": "Unauthorized" }`** if there is no session (e.g. [`summary/route.ts:15-18`](../../../src/app/api/wise-activity/summary/route.ts)). There is **no** `CRON_SECRET` path and **no** machine-token path on these five — they are admin-session only.
  - Page-level access control also applies in the middleware: a restricted admin whose `allowedPages` does not cover `/wise-activity` is denied at the edge with **HTTP 403 `{ "error": "Forbidden" }`** for any `/api/...` path ([`isPathAllowed`, middleware.ts:25-35, :53-56](../../../src/middleware.ts)). Full-access admins (`allowedPages === null`) short-circuit to allowed.
- **Endpoint 6** lives under `/api/internal/`, which the middleware treats as a **public route** (no session redirect, [`middleware.ts:13`](../../../src/middleware.ts)). It is protected solely by a `CRON_SECRET` bearer check via the shared `rejectInvalidCronSecret` helper ([`sync-wise-activity/route.ts:13-14`](../../../src/app/api/internal/sync-wise-activity/route.ts); [`cron-auth.ts:19-26`](../../../src/lib/internal/cron-auth.ts)).
...
The two date strings are converted to an inclusive UTC instant window by `wiseActivityBangkokRange(startDate, endDate)`: `start = bangkokDateStartUtc(startDate)`, `end = bangkokDateStartUtc(endDate + 1 day) − 1 ms` ([`data.ts:240-245`](../../../src/lib/wise-activity/data.ts)). All filtering is then a `BETWEEN start AND end` on `wise_activity_events.eventTimestamp` ([`data.ts:99-102`](../../../src/lib/wise-activity/data.ts)).
...
Empty/whitespace-only string params are coerced to `undefined` (ignored) by `stringParam` ([`route.ts:16-19`](../../../src/app/api/wise-activity/route.ts)). `financeOnly` is true only for the exact literal string `"true"` ([`route.ts:48`](../../../src/app/api/wise-activity/route.ts), [`summary/route.ts:41`](../../../src/app/api/wise-activity/summary/route.ts)).
...
**Request — query params:** `startDate`, `endDate` (shared date semantics above), plus the shared filter set (`type`, `eventName`, `q`, `sessionId`, `transactionId`, `financeOnly`). No body. The handler forwards them to `getWiseActivitySummary` ([`summary/route.ts:29-43`](../../../src/app/api/wise-activity/summary/route.ts) → [`data.ts:163-238`](../../../src/lib/wise-activity/data.ts)), which loads **all** matching rows (no pagination) plus the single most-recent `wise_activity_sync_runs` row.
...
| `cards.sessionMutationEvents` | number | rows whose `eventName` is one of the four session-mutation events ([`format.ts:3-8`](../../../src/lib/wise-activity/format.ts), [`isWiseSessionMutation`, format.ts:68-70](../../../src/lib/wise-activity/format.ts)) |
...
**Side effects:** none (two `SELECT`s only — the matched events plus the latest sync run, run in parallel, [`data.ts:166-177`](../../../src/lib/wise-activity/data.ts)).
...
(Column-level detail for the backing `wise_activity_events` table lives at [`schema.ts:245-278`](../../../src/lib/db/schema.ts) — not restated here.)

**Side effects:** none (count + page `SELECT`, run in parallel, [`data.ts:137-149`](../../../src/lib/wise-activity/data.ts)).
...
**Auth:** Auth.js session (401 if absent). **Does:** cross-check the active Sales Dashboard source's package-sale rows against Wise receipt transactions, plus a sheet-vs-Wise revenue variance. Read-only. (Meaning, scoring rationale, and the review-only guarantee: see the [feature doc](../../features/wise-activity-audit.md).)

**Request — query params** (all optional; parsed via `stringParam`, [`reconciliation/route.ts:26-30`](../../../src/app/api/wise-activity/reconciliation/route.ts)):
...
**What it reads** (`getWisePackageSalesReconciliation`, [`reconciliation.ts:780-880`](../../../src/lib/wise-activity/reconciliation.ts)): the non-archived Sales Dashboard sources; the selected source's normalized package-sale rows; date-windowed inbound Wise invoice/payment events from `wise_activity_events` (BILLING / `transactionId` / invoice·payment·transaction name, excluding `%payout%`, [`reconciliation.ts:829-836`](../../../src/lib/wise-activity/reconciliation.ts)); the active Credit Control snapshot's packages; and — **via live Wise API calls** — `fetchWiseFeesPaidTrends` and `fetchWiseReceiptTransactions` ([`reconciliation.ts:846-849`](../../../src/lib/wise-activity/reconciliation.ts)). If `WISE_USER_ID`/`WISE_API_KEY` are unset, or a Wise call throws, the corresponding block degrades gracefully to an `error` string with `*Available: false` flags rather than failing the request ([`reconciliation.ts:725-778`](../../../src/lib/wise-activity/reconciliation.ts)).
...
| `selectedSource` | `ReconciliationSourceSummary \| null` | resolved by `sourceId` → `month` → most-recent-successful fallback ([`selectSource`, reconciliation.ts:716-723](../../../src/lib/wise-activity/reconciliation.ts)) |
...
| `revenueVariance` | `ReconciliationRevenueVariance` | sheet total vs Wise fees-paid trend vs receipts, currency `THB` ([`reconciliation.ts:135-156`](../../../src/lib/wise-activity/reconciliation.ts)) |
...
> **Empty-state shortcut:** if no source can be selected, the endpoint returns a fully-formed but empty reconciliation (empty arrays, today's Bangkok date as the range) rather than an error ([`reconciliation.ts:791-801`](../../../src/lib/wise-activity/reconciliation.ts)).

**Side effects:** none persisted. It **does** make outbound read-only calls to the Wise API for fees-paid trends and receipts (no Wise mutation).
...
Endpoints 4 and 5 both drive the **same** ingest function `syncWiseActivityEvents(db, client, instituteId, options)` ([`sync.ts:139-275`](../../../src/lib/wise-activity/sync.ts)) with `triggerType: "manual"`. The institute id is `process.env.WISE_INSTITUTE_ID ?? "696e1f4d90102225641cc413"`. Both declare `export const maxDuration = 800` ([`sync/route.ts:7`](../../../src/app/api/wise-activity/sync/route.ts), [`reconciliation/backfill/route.ts:8`](../../../src/app/api/wise-activity/reconciliation/backfill/route.ts)).

**Shared single-flight guard:** a partial unique index `wise_activity_sync_runs_single_running_idx` (`status = 'running'`) lets at most one `running` row exist ([`schema.ts:293-296`](../../../src/lib/db/schema.ts)). A concurrent insert raises a unique violation that is caught and rethrown as `WiseActivitySyncAlreadyRunningError` ([`sync.ts:38-43, :154-168`](../../../src/lib/wise-activity/sync.ts)), which **both** handlers map to **HTTP 409 `{ "error": "Wise activity sync is already running" }`**. Stale `running` rows older than 20 minutes (`STALE_RUNNING_MS`) are reaped to `failed` at the start of every run ([`sync.ts:13, :117-129, :151`](../../../src/lib/wise-activity/sync.ts)).
...
**Shared side effects:** inserts a `wise_activity_sync_runs` row (`status: "running"`) on start; pages Wise newest-first via `fetchWiseActivityEvents` (`PAGE_SIZE = 50`); normalizes (`normalizeWiseActivityEvent`, dropping rows without an `eventId` or `eventTimestamp`) and **idempotently** inserts events with `onConflictDoNothing` on `eventId` ([`sync.ts:191-216`](../../../src/lib/wise-activity/sync.ts)); finally updates the run row to `success` (or `failed` with `errorSummary` on throw, [`sync.ts:232-273`](../../../src/lib/wise-activity/sync.ts)).
...
**Request body** (read with a `try/catch` that defaults to `{}`; non-object bodies coerced to `{}`, [`sync/route.ts:22-28`](../../../src/app/api/wise-activity/sync/route.ts)). Both fields optional, each passed through `numberOption` (must be an integer, else fallback; then clamped to range, [`sync/route.ts:11-14`](../../../src/app/api/wise-activity/sync/route.ts)):
...
(Wired at [`sync/route.ts:37-38`](../../../src/app/api/wise-activity/sync/route.ts).)
...
**Does:** the same activity ingest as endpoint 4, but with the lookback **derived from a required start date** so finance staff can pull enough history to make a reconciliation range's inbound events `complete`.

**Request body** (required date range; same `try/catch` → `{}` parsing, [`reconciliation/backfill/route.ts:24-32`](../../../src/app/api/wise-activity/reconciliation/backfill/route.ts)):
...
**Auth:** `CRON_SECRET` bearer only (no session). **Schedule:** `5,35 * * * *` per [`vercel.json:23-26`](../../../vercel.json) (registry `cadenceLabel: "Every 30 min"`, [`cron-registry.ts:51-61`](../../../src/lib/data-health/cron-registry.ts)). **Does:** the same `syncWiseActivityEvents` ingest as the manual triggers, but with `triggerType: "cron"` and tighter bounds. **Read-only with respect to Wise; never touches snapshot data.**

**Auth check:** `rejectInvalidCronSecret(request)` runs first ([`sync-wise-activity/route.ts:13-14`](../../../src/app/api/internal/sync-wise-activity/route.ts)) — returns **HTTP 401 `{ "error": "Unauthorized" }`** on a bad/absent bearer, or **HTTP 500 `{ "error": "Server misconfigured" }`** when `CRON_SECRET` itself is unset ([`cron-auth.ts:19-26`](../../../src/lib/internal/cron-auth.ts)). The comparison is constant-time with a length pre-check ([`cron-auth.ts:6-17`](../../../src/lib/internal/cron-auth.ts)).
...
**Response (HTTP 200):** `{ "ok": true, "result": WiseActivitySyncResult }` — same `WiseActivitySyncResult` shape as the manual triggers, but `result.triggerType === "cron"` ([`sync-wise-activity/route.ts:20-26`](../../../src/app/api/internal/sync-wise-activity/route.ts)).

**Side effects:**
- The activity ingest itself (sync-run ledger row + idempotent `wise_activity_events` inserts), bounded to the 3-day / 20-page cron window.
- The whole handler is wrapped in `withCronInvocationAudit({ jobKey: "wise_activity", triggerSource: "cron", requestMethod })` ([`sync-wise-activity/route.ts:16-35`](../../../src/app/api/internal/sync-wise-activity/route.ts)), which writes a `cron_invocations` audit row (`outcome: "running"` on start, then `success`/`skipped`/`failed` with `durationMs`, `responseStatus`, and a `linkedRunIds.syncRunId` extracted from the response body, [`cron-audit.ts:84-159`](../../../src/lib/data-health/cron-audit.ts)). A 409 "already running" response is recorded as outcome `skipped` ([`cron-audit.ts:61-70`](../../../src/lib/data-health/cron-audit.ts)).

**Errors:** 401 / 500 (auth, above); **409** `Wise activity sync is already running` ([`sync-wise-activity/route.ts:28-29`](../../../src/app/api/internal/sync-wise-activity/route.ts)); 500 `{ "error": <message ?? "Wise activity sync failed"> }` ([`sync-wise-activity/route.ts:30-32`](../../../src/app/api/internal/sync-wise-activity/route.ts)).

> The scheduling, stagger, and stop-condition mechanics for this cron are covered in depth in [`reference/crons.md`](../crons.md) and are not duplicated here.
