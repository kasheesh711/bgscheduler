...
- **Whether any of this is applied in production is unknowable from the repo.** `main` declares 36 tables; migrations `0053`/`0054` are present in `drizzle/` and journaled (`drizzle/meta/_journal.json`, entries 53–54 of 69). Whether they have been run against the Neon database is a runtime fact — see the feature page's [open questions](../../features/university-admissions.md#open-questions).
