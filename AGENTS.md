...
| 22 | Student Schedule | stable | Builds a student's monthly calendar from `credit_control_sessions` on the active credit-control snapshot, exports A4/PDF, and delivers it to a parent over LINE via a capability-token link at the public `/schedule/{token}` page — no account, no login. Admin preview and print report render the shared month calendar; the parent page renders a phone-first Thai agenda from the **same payload**. Cancelled sessions omitted; an unresolved teacher renders `TEACHER_TBC`, never inferred. 1 table, 2 endpoints. | [student-schedule](docs/features/student-schedule.md) |
